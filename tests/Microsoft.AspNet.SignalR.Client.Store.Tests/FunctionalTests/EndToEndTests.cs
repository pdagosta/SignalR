        ...
        public async Task WebSocketSendReceiveTest()
        {
            ...
            try
            {
                using (var hubConnection = new HubConnection(HubUrl))
                {
                    ...
                    hubConnection.TraceWriter = new StringWriter(traces);

                    hubConnection.Error += e =>
                    {
                        throw new InvalidOperationException("Traces: " + traces.ToString(), e);
                    };
            ...
            {
                // very poor man's tracing
                throw new InvalidOperationException(traces.ToString(), exception);
            }
        }
